...
func unpackName(b []byte, start int) (int, []string, bool) {
	strArr := []string{}
	i := start
	for b[i] != 0 {
		if b[i]&0xc0 != 0 { //a pointer
			return i - start, strArr, true
		}
		strArr = append(strArr, string(b[i+1:int(b[i])+i+1]))
		i += int(b[i]) + 1
	}
	return i - start, strArr, false
}
	...
	for i := 0; i < int(req.ANcount); i++ { // loop over questions
		...
		for b[pos] != 0 {
			...
			if b[pos]&0xc0 != 0 { //a pointer
				...
				_, name, _ := unpackName(b, offset)
				pos += 2
				rr.Name = append(rr.Name, name...)
				break
			} else {
				l, name, _ := unpackName(b, pos)
				pos += l
				rr.Name = append(rr.Name, name...)
			}
		}
