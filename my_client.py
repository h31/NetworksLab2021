    ...
    while True:
        if buffer == 0:
            ...
            if message.strip() == "file~~".encode(code_table):
                ...
                if "~~".encode(code_table) in file_data_write:
                    f.write(file_data_write)
                    f.close()
                    print("clF1")
                else:
                    while not "~~".encode(code_table) in file_data_write:
                        full_file += file_data_write
                        file_data_write = client.recv(file_size)
                    else:
                        full_file += file_data_write
                        f.write(full_file[:full_file.find("~~".encode(code_table))])
                        f.close()
                        print("clF2")
...
def send_message():
    while True:
        ...
        if message == "file~~":
            ...
            file_size = os.path.getsize(file_name) + len('~~'.encode(code_table)) - 1000
            ...
            mod_data_to_send = file_data_write + '~~'.encode(code_table)
            client.send(mod_data_to_send)
            f.close()
            print(f'File {file_name} is send')
        else:
            message_send = f'<{local_tz}>{name}->{message}'.encode(code_table)
            message_len_send = f'{len(message_send) - 1000:<{length_of_message}}'.encode(code_table)
